...
class ViewController: NSViewController, NSURLSessionDownloadDelegate {
    ...
    var firstFrameInt: Int = 0; var lastFrameInt: Int = 0; var cFrame: Int = 0;
    ...
    @IBAction func renderButtonClicked(sender: AnyObject) {
        ...
        if !animation {
            ...
            firstFrameInt = Int(firstFrameNumber.stringValue)!;
            lastFrameInt = Int(lastFrameNumber.stringValue)!;
            /* let firstFrameInt = Int(firstFrameNumber.stringValue)!;
            let lastFrameInt = Int(lastFrameNumber.stringValue)!;
            for i in firstFrameInt...lastFrameInt {
            var imageBaseName = "";
            if i < 10 {
            imageBaseName = blendFileSimple + "_out000" + String(i);
            } else if i < 100 {
            imageBaseName = blendFileSimple + "_out00" + String(i);
            } else if i < 1000 {
            imageBaseName = blendFileSimple + "_out0" + String(i);
            } else {
            imageBaseName = blendFileSimple + "_out" + String(i);
            }
            
            var imageFileName = "not ready";
            let fileManager = NSFileManager.defaultManager();
            let enumerator = fileManager.enumeratorAtPath(outFolder);
            while imageFileName == "not ready" {
            while let element = enumerator?.nextObject() as? String {
            if element.containsString(imageBaseName) {
            imageFileName = element;
            }
            ...
            imageViewer.image = NSImage(byReferencingFile: image);
            let imageArray = image.componentsSeparatedByString("/");
            statusBar.stringValue = "Rendering frames " + String(firstFrameInt) + " to " + String(lastFrameNumber) + ". Displaying " + imageArray[imageArray.count - 1]
            }
            renderTask.waitUntilExit(); */
            
            /* dispatch_async(dispatch_get_global_queue(Int(QOS_CLASS_UTILITY.rawValue), 0)) {
                for i in self.firstFrameInt...self.lastFrameInt {
                    var imageBaseName = "";
                    if i < 10 {
                        imageBaseName = self.blendFileSimple + "_out000" + String(i);
                    } else if i < 100 {
                        imageBaseName = self.blendFileSimple + "_out00" + String(i);
                    } else if i < 1000 {
                        imageBaseName = self.blendFileSimple + "_out0" + String(i);
                    } else {
                        imageBaseName = self.blendFileSimple + "_out" + String(i);
                    }
                    
                    var imageFileName = "not ready";
                    while imageFileName == "not ready" {
                        var fileManager: NSFileManager? = NSFileManager.defaultManager();
                        var enumerator: NSEnumerator? = fileManager!.enumeratorAtPath(self.outFolder);
                        while let element = enumerator!.nextObject() as? String {
                            if element.containsString(imageBaseName) {
                                imageFileName = element;
                            }
                    ...
                    dispatch_async(dispatch_get_main_queue()) {
                        let image = self.outFolder + "/" + imageFileName;
                        self.imageViewer.image = NSImage(byReferencingFile: image);
                        let imageArray = image.componentsSeparatedByString("/");
                        self.statusBar.stringValue = "Rendering frames " + String(self.firstFrameInt) + " to " + String(self.lastFrameInt) + ". Displaying " + imageArray[imageArray.count - 1]
                    }
                }
            } */
            
            cFrame = firstFrameInt;
            uiUpdateTimer = NSTimer.scheduledTimerWithTimeInterval(1, target: self, selector: Selector("updateUI"), userInfo: nil, repeats: true);
        }
    }
    ...
    func updateUI() {
        var imageBaseName = "";
        if cFrame < 10 {
            imageBaseName = blendFileSimple + "_out000" + String(cFrame);
        } else if cFrame < 100 {
            imageBaseName = blendFileSimple + "_out00" + String(cFrame);
        } else if cFrame < 1000 {
            imageBaseName = blendFileSimple + "_out0" + String(cFrame);
        } else {
            imageBaseName = blendFileSimple + "_out" + String(cFrame);
        }
        
        var imageFileName = "not ready";
        let enumerator = NSFileManager.defaultManager().enumeratorAtPath(outFolder);
        while let element = enumerator?.nextObject() as? String {
            if element.containsString(imageBaseName) {
                imageFileName = element;
            }
        ...
        if imageFileName != "not ready" {
            cFrame = cFrame + 1;
            let image = outFolder + "/" + imageFileName;
            imageViewer.image = NSImage(byReferencingFile: image);
            let imageArray = image.componentsSeparatedByString("/");
            if cFrame > lastFrameInt {
                statusBar.stringValue = "Ready. Rendered " + String(lastFrameInt - firstFrameInt) + " frames. Displaying " + imageArray[imageArray.count - 1];
                uiUpdateTimer.invalidate();
            } else {
                statusBar.stringValue = "Rendering frame " + String(cFrame) + " (of " + String(firstFrameInt) + " to " + String(lastFrameInt) + "). Displaying " + imageArray[imageArray.count - 1];
            }
        }
    }
